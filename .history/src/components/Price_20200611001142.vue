    ...
    <div class="public bottom">
      ...
      <el-card class="box-cardr public">
        <img
          ...
          alt
        />
        <!-- <div v-for="item in cations" :key="item.id">
          <div class="cation">{{item.name}}</div>
          <el-progress :percentage="item.progress" status="success" v-if="item.progress === 1"></el-progress>
        </div>-->

        <div v-for="item in cations" :key="item.name">
          {{ item.name }}
          <el-progress :percentage="item.progress" status="success" :format="format" v-if="item.progress === 1"></el-progress>
  ...
  methods: {
    ...
    getProgress() {
      axios
        .get("api/progress")
        .then(res => {
          this.cations = res.data.data;
          console.log(this.cations);
        })
        .catch(err => {
          console.log(err);
        });
